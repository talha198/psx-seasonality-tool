...
custom_ticker = st.sidebar.text_input("Enter Custom PSX Ticker (e.g. HBL.KS)", value="HBL.KS")
start_date = st.sidebar.date_input("Start Date", value=pd.to_datetime("2015-01-01"))
end_date = st.sidebar.date_input("End Date", value=pd.to_datetime("today"))

predefined = st.sidebar.checkbox("Use 5 Predefined PSX Stocks", value=False)
# Example tickers with '.KS' suffix (verify these on Yahoo Finance)
predefined_tickers = ["TRG.KS", "OGDC.KS", "ENGRO.KS", "HBL.KS", "LUCK.KS"]

@st.cache_data(show_spinner=False)
def fetch_stock_data(ticker, start, end):
    df = yf.download(ticker, start=start, end=end, progress=False, auto_adjust=True)
    if df.empty:
        raise ValueError(f"No data found for ticker '{ticker}'. Please verify the ticker symbol and try again.")
...
def display_seasonality_for_ticker(ticker, start, end):
    try:
        ...
        fig.add_trace(go.Bar(
            ...
            y=forecast_returns,
            marker_color=['lime' if x >= 0 else 'tomato' for
